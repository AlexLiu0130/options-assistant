...
export function StrategySimulator({
  ...
  return (
    <div className="ticket-simulator">
      ...
      <input value={pricePct} onChange={(event) => setPricePct(Number(event.target.value))} type="range" min="0" max="100" style={{ '--pct': `${pricePct}%` } as CSSProperties} />
      <strong>
        {date.toLocaleDateString(lang === 'zh' ? 'zh-CN' : 'en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
        <em>{daysLeft} DTE</em>
      </strong>
      <input value={dayPct} onChange={(event) => setDayPct(Number(event.target.value))} type="range" min="0" max="100" style={{ '--pct': `${dayPct}%` } as CSSProperties} />
