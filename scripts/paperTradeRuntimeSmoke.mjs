...
const opened = await submitPaperOrder({ ticker: 'TST', strategySnapshot: strategy, underlyingPrice: 100 }, Date.UTC(2026, 6, 1, 14))
assert.equal(opened.status, 201)
assert.equal(opened.body.order.status, 'filled')
assert.match(opened.body.order.id, uuidPattern)
assert.ok(opened.body.position?.id)
assert.match(opened.body.position.id, uuidPattern)
...
const unmarkedAccount = await getPaperAccount()
...
const marked = await listPaperPositions({
  status: 'open',
...
const markedAccount = await getPaperAccount({ prices: { TST: 120 } }, Date.UTC(2026, 6, 2, 14))
...
const closed = await closePaperPositionById(
...
const afterClose = await getPaperAccount()
...
const closedMarket = await submitPaperOrder({ ticker: 'TST', strategySnapshot: strategy, underlyingPrice: 100 }, Date.UTC(2026, 6, 1, 2))
assert.equal(closedMarket.status, 422)
assert.match(closedMarket.body.order.rejectReason, /MARKET_CLOSED/)

await resetPaperAccount({ initialCash: 100 })
const creditRisk = await submitPaperOrder(
  {
    ticker: 'TST',
    strategySnapshot: {
      id: 'cash-risk',
...
const fractional = await submitPaperOrder({ ticker: 'TST', strategySnapshot: strategy, underlyingPrice: 100, quantity: 1.5 }, Date.UTC(2026, 6, 1, 14))
assert.equal(fractional.status, 422)
assert.match(fractional.body.order.rejectReason, /positive integer/)

const badLeg = await submitPaperOrder(
  {
    ticker: 'TST',
...
await resetPaperAccount({ initialCash: 2000 })
const spoofedRisk = await submitPaperOrder(
  {
    ticker: 'TST',
    strategySnapshot: {
      id: 'spoof-risk',
...
const reset = await resetPaperAccount({ initialCash: 250000 })
assert.equal(reset.body.account.initialCash, 250000)
assert.equal(reset.body.positions.length, 0)
assert.equal(reset.body.clearedOrders, 1)
assert.equal(reset.body.clearedPositions, 0)
assert.equal((await listPaperOrders()).body.orders.length, 0)

await resetPaperAccount({ initialCash: 111111 }, Date.now(), 'oauth-user-a')
await resetPaperAccount({ initialCash: 222222 }, Date.now(), 'oauth-user-b')
assert.equal((await getPaperAccount({}, Date.now(), 'oauth-user-a')).body.account.initialCash, 111111)
assert.equal((await getPaperAccount({}, Date.now(), 'oauth-user-b')).body.account.initialCash, 222222)
