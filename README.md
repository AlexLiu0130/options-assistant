...
QVERIS_OAUTH_REDIRECT_URI=http://localhost:8787/auth/callback
...
Use the same hostname for the frontend URL and `QVERIS_OAUTH_REDIRECT_URI`. Browsers treat `localhost` and `127.0.0.1` as different cookie hosts, so mixing them causes the OAuth callback to fail state validation.
